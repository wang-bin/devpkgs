...
cp -avf install/x64/include/{ass,GLFW,mfx,vpl,*.h} dep/include
for A in arm64 x64 x86; do
    mv install/$A/bin/{libass,zlib}.dll dep/bin/windows/$A/
    mv install/$A/share/pkgconfig/zlib.pc dep/lib/windows/$A/pkgconfig            # TODO: edit zlib.pc
    mv install/$A/lib/{glfw3,snappy,va*,zlib*}.lib dep/lib/windows/$A/MD/
...
for A in x64 x86; do
    mv install/$A/bin/{libass,zlib}.dll dep/bin/windows/$A/LTL
    mv install/$A/lib/{glfw3,mfx,snappy,vpl,zlib*}.lib dep/lib/windows/$A/MT/
done
rm -rf install

7z x -y devpkgs-uwp-Release-vs2022.7z
for A in x64 arm64; do
    mv install/$A/bin/{libass,zlib}.dll dep/bin/WinRT/$A
    mv install/$A/lib/{snappy,zlib*}.lib dep/lib/WinRT/$A
done
rm -rf install
